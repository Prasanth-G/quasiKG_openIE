...
"""
A simple Python wrapper for the stanford IE binary that makes it easier touseit
...
def stanford_ie(input_filenames, verbose=True, generate_graphviz=False):
    ...
    command += 'cd {}; {} -mx4g -cp "stanford-openie.jar:stanford-openie-models.jar:lib/*" ''edu.stanford.nlp.naturalli.OpenIE {} -format ollie > {}'.format(STANFORD_IE_FOLDER, JAVA_BIN_PATH, new_filename, out)
...
def main(args):
    ...
    entities_relations = stanford_ie(filename, True, True)

    print('\n'.join([' |'.join(a) for a in entities_relations]))


if __name__ == '__main__':
    filenames = [os.path.join('corpus', each) for each in os.listdir('corpus') if each.endswith('.txt')]
    # filename = "corpus//pg6130.txt"
    entities_relations = stanford_ie(filenames, True, True)
    print(len(entities_relations))
    print(entities_relations[:10])